...
const ChatInterface = () => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        const hasShownPopup = await AsyncStorage.getItem(keyFor("hasShownApiKeyPopup"));
        
        // Check if we have any valid API keys
        const hasValidKeys = PROVIDERS.some(p => (keys[p] || "").trim() !== "");
        ...
        if (selected && (keys[selected] || "").trim() !== "") {
          ...
          const firstValid = PROVIDERS.find(p => (keys[p] || "").trim() !== "");
  ...
  const handleDownloadChat = async () => {
    ...
    const generateHTMLFromMessages=(messages)=>{
      ...
      const memberName = selectedMember ? `${selectedMember.firstName} ${selectedMember.lastName}` : 'Unknown Member' ;
  ...
  `;
    ...
    const messageText = msg.text.replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]*>/g, '');
  ...
  const saveKeys = async () => {
    try {
      ...
      if (!validProvider || (newKeys[validProvider] || "").trim() === "") {
        validProvider = PROVIDERS.find((p) => (newKeys[p] || "").trim() !== "") || "";
    ...
    <View style={styles.container}>
                    ...
                    >
                      {isRenamingChatId === chat.id ? (
                        <TextInput
                          ...
                          onBlur={() => handleRenameChat(chat.id, renameInput)}
                          autoFocus
        ...
        <View style={styles.mainContent}>
          ...
          <View style={styles.inputArea}>
            <TextInput
              ...
              editable={!!selectedAPI && !!(apiKeys[selectedAPI] || "").trim()}
      ...
      <Modal visible={isSettings} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          ...
          <Text style={styles.modalTitle}>Current AI Providers and API Keys</Text>
          ...
          <ScrollView style={styles.apiProvidersList}>
            {PROVIDERS.map((provider) => {
              ...
              return (
                <View
                  key={provider}
                  style={[styles.apiProvider, key && styles.apiProviderWithKey]}
                >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              To get started, please enter at least one API key for your preferred AI provider.
            </Text>
